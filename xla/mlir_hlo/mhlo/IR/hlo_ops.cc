...
Type maybeTupleFromTypes(MLIRContext *ctx, ArrayRef<Type> types) {
  if (types.size() == 1 && !types[0].isa<TupleType>())
    return types[0];
...
LogicalResult
AsyncUpdateOp::inferReturnTypes(MLIRContext *, std::optional<Location>,
                                ValueRange operands, DictionaryAttr attributes,
                                OpaqueProperties, RegionRange regions,
...
LogicalResult
ConcatenateOp::inferReturnTypes(MLIRContext *, std::optional<Location> location,
                                ValueRange operands, DictionaryAttr attributes,
                                OpaqueProperties, RegionRange regions,
                                SmallVectorImpl<Type> &inferredReturnTypes) {
...
static bool isEligibleForCompactPrint(ReduceOp op) {
  ...
  if (blockArgLoc != block.getArgument(1).getLoc())
    return false;
...
eliminateBroadcastInDimTranspose(TransposeOp op, PatternRewriter &rewriter) {
...
ScatterOp::fold(FoldAdaptor adaptor,
  ...
  if (baseType.getElementType().isa<ComplexType>())
    return failure();
  ...
  do {
    ...
    for (uint64_t i = 0; i < baseIndex.size(); ++i) {
      if (llvm::count(insertedWindowDims, i))
        continue;
